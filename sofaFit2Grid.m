...
%                - Para "copiar" o grid de um objeto para outro, defina out_pos
%                como Obj.SourcePosition, em que Obj é o objeto SOFA
%                contendo as posições objetivo.
...
%                força a assumirem suas coordenadas. (Padrão)
%    'hybrid':   Faz a adaptação como em 'adapt', mas posições do grid
%                original escolhidas para mais de uma posição objetivo 
%                são determinadas por interpolação.
%    'vbap':     Interpolação vbap
%    'bilinear': Interpolação bilinear 
%    'spherical_harmonics': Interpolação por meio de harmônicos esféricos
%                           SUpDEq API necessária: https://github.com/AudioGroupCologne/SUpDEq
%    'Fs':       Transformação da taxa de amostragem no objeto de saída 
%                (Padrão: original do objeto).
